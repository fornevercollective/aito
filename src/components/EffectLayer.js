...
const UNFOCUSED_TYPE = { linear: 0, quadratic: 1, cubic: 2, square: 3 };
const UNFOCUSED_BLEND = { none: 0, darker: 1, lighter: 2, both: 3 };
const GLASS_PATTERN = { lens: 0, ripple: 1, splat: 2 };
const CRACK_TYPE = {
    classic: 0,
    modern: 1,
};
export function EffectLayer({ layer, texture, order, clipSign, clipX }) {
    ...
    useFrame(() => {
        ...
        switch (layer.kind) {
            ...
            case "unfocused": {
                ...
                u.uOrigin.value.set(p.originX, p.originY);
            ...
            case "burn": {
                ...
                u.uDispersion.value = p.dispersion;
                u.uEdgeColor.value.setRGB(...p.edgeColor);
                u.uMaskColor.value.setRGB(...p.maskColor);
    ...
    return (_jsxs("mesh", { position: [0, 0, order * 0.001], children: [_jsx("planeGeometry", { args: [1, 1] }), _jsx("shaderMaterial", { ref: matRef, vertexShader: spec.vert, fragmentShader: spec.frag, uniforms: uniforms, transparent: true })] }));
}
