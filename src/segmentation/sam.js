...
export async function ensureSam() {
    if (loadState === "ready" && model && processor)
        return true;
    ...
    try {
        const loaded = (await SamModel.from_pretrained(MODEL_ID, {
            dtype: "fp32",
            device: "webgpu",
        }).catch(() => SamModel.from_pretrained(MODEL_ID, { dtype: "fp32" })));
        model = loaded;
...
async function getEmbeddings(imageUrl) {
    if (!model || !processor)
        throw new Error("SAM not loaded");
    if (embedCache?.url === imageUrl)
        return embedCache;
...
function tensorMaskToDataUrl(maskTensor, maskIndex, width, height) {
    const [, , h, w] = maskTensor.dims;
    const data = maskTensor.data;
    const plane = maskIndex * h * w;
    const rgba = new Uint8ClampedArray(w * h * 4);
    let minX = w, minY = h, maxX = 0, maxY = 0, sx = 0, sy = 0, n = 0;
    for (let y = 0; y < h; y++) {
        for (let x = 0; x < w; x++) {
            const i = plane + y * w + x;
...
export async function segmentAtPoint(imageUrl, nx, ny) {
    const ok = await ensureSam();
    if (!ok || !model || !processor)
        return null;
    const raw = await RawImage.read(imageUrl);
    const w = raw.width;
    const h = raw.height;
    ...
    const { dataUrl, bbox, centroid } = tensorMaskToDataUrl(masks[0], best, w, h);
...
export async function autoSegmentSubjects(imageUrl, maxMasks = 6) {
    const ok = await ensureSam();
    if (!ok || !model || !processor)
        return [];
    const raw = await RawImage.read(imageUrl);
    ...
    for (let gy = 1; gy < grid; gy++) {
        for (let gx = 1; gx < grid; gx++) {
            ...
            if ((scores[best] ?? 0) < 0.75)
                continue;
            const { dataUrl, bbox, centroid } = tensorMaskToDataUrl(masks[0], best, w, h);
            if (bbox.w * bbox.h < 0.02)
                continue;
            const dup = results.some((r) => Math.abs(r.centroid.x - centroid.x) < 0.08 &&
