...
import { getAllPresetSlugs } from './lutPresets';

export interface GrokAction {
  tool: 'set_adjustments' | 'apply_lut' | 'create_mask' | 'set_mask_scope' | 'capture_tether' | 'add_bake_note';
...
export interface GrokEditResponse {
...
- apply_lut: preset slug from the imagine catalog (e.g. "kodak-portra-400", "dark-academia", "teal-orange-blockbuster", "bleach-bypass-lut", "fuji-eterna-500", "anamorphic-cinematic" etc). Full list in src/data/imagine-presets.json (source: /Users/qbit/dev/imagine/style_presets)
...
export async function callGrokForEdits(
  apiKey: string,
  prompt: string,
  imageBase64: string,
  currentAdjustments: any,
  referenceImages: string[] = [], // additional reference board images
  conversationHistory: any[] = []
): Promise<GrokEditResponse> {
  ...
  if (message?.tool_calls?.length > 0) {
    return {
      plan: message.tool_calls.map((tc: any) => ({
...
export async function askGrokForPlan(
  apiKey: string,
  prompt: string,
  imageBase64: string,
  currentAdjustments: any,
  referenceImages: string[] = []
): Promise<GrokPlan | null> {
...
Return ONLY this JSON:
{
  ...
  if (!res.ok) throw new Error(`Grok plan error: ${res.status}`);
...
export function captureCurrentImageBase64(): string {
...
export function executeGrokPlan(
  plan: GrokAction[],
  dispatchers: {
    setAdjustment: (key: string, value: number) => void;
    applyLut: (preset: string, intensity?: number) => void;
    createMaskFromPrompt: (description: string) => void;
    setMaskScope: (scope: 'subject' | 'background' | 'all') => void;
    captureTether?: () => void;
  }
) {
      ...
      switch (step.tool) {
        case 'set_adjustments':
          Object.entries(step.args.adjustments || {}).forEach(([key, val]) => {
            if (typeof val === 'number') dispatchers.setAdjustment(key, val);
          });
          break;
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          preset: {
            type: "string",
            description: "Canonical slug from the imagine style catalog at /Users/qbit/dev/imagine/style_presets (e.g. kodak-portra-400, teal-orange-blockbuster, bleach-bypass-lut, dark-academia, kodak-vision3-500t, ...). See src/data/imagine-presets.json for the live list.",
            enum: getAllPresetSlugs(),
          },
