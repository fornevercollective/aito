# aito-living-canvas — handoff

Workspace: `~/dev/aito-living-canvas`. Living Canvas pivot of aito.

## Spatial multi-source (2026-07)

Shared with **aito-mac booth** + Splatline:

- `src/lib/spatial-depth.ts` — nested sphere shells, parallax styles,
  multi-source mix weights, hand → depth/wave mods.
- `src/styles/living-tokens.css` — `--lc-sphere-*`, depth near/far, hand vars.
- aito-mac (localhost booth): selection, depth variation modes, voxel sphere,
  hand-guided waveform/depth, webcam/mobile/screen/live sources.
- **Hand resources (2026-07-10):** MediaPipe **0.10.35**, gesture bus
  (open/fist/pinch/point/peace + two-hand span), catalog at
  `~/dev/aito-mac/booth/hand-tracking-resources.mjs` (SkalskiP, measure_plan,
  Splatline v2, Image_to_Mesh, Cells2Pixels, DBR-XR/prism-ai, Mirelo, Three.js).

# aito — handoff (upstream)
