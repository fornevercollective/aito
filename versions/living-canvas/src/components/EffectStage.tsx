...
function FitQuad({ children }: { children: React.ReactNode }) {
  ...
  return (
    <group scale={[size.width, size.height, 1]}>{children}</group>
  );
}
  ...
  return (
    <>
      ...
      <mesh position={[0, 0, -0.003]}>
        <planeGeometry args={[1, 1]} />
        <shaderMaterial
          vertexShader={ADJUST_VERT}
          fragmentShader={ADJUST_FRAG}
          uniforms={{
            uMap: { value: afterTex },
      ...
      {layers
        ...
        .map((l, i) => {
          ...
          const clipSign: -1 | 0 | 1 =
            l.side === "both" ? 0 : l.side === "before" ? -1 : 1;
          return (
            <EffectLayer
              key={l.id}
              layer={l}
...
export function EffectStage() {
  return (
    <Canvas
      ...
      camera={{ position: [0, 0, 100], near: -1000, far: 1000 }}
      dpr={[1, 2]}
    >
